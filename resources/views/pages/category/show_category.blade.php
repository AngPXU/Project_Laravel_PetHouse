			...
			<div class="col-lg-9 col-md-9">
				<div class="row">
					<div class="col-lg-12 col-md-12">
						<div class="section-title">
							<h4>Danh mục: @foreach($category_name as $key => $name)
								{{$name->category_name}}
							@endforeach</h4>
							<a><button class="btn btn-warning" type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
								<i class="fas fa-filter"></i>&ensp;Lọc theo danh mục
							</button></a>
								...
								<div class="card-body">
									...
									@foreach($category as $key => $cate)
										<div class="form-check form-check-inline" style="padding-left:5px;padding-right:5px;">
											<input class="form-check-input category-filter" type="checkbox" name="category-filter" value="{{$cate->category_id}}" 
											data-filters="category" {{in_array($cate->category_id, $category_arr) ? 'checked' : ''}}>
											<label class="form-check-label">{{$cate->category_name}}</label>
										</div>
									@endforeach
								</div>
							  </div>
				...
				<div class="row">
					<div class="col-12">
						<ul class="pagination justify-content-center">
							<li class="page-item">{{$category_by_id->links()}}</li>
						</ul> 
					</div>
				</div>
			</div>
